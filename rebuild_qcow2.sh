...
if [[ "$qcow_file" == *"debian"* || "$qcow_file" == *"ubuntu"* || "$qcow_file" == *"arch"* ]]; then
    ...
    virt-customize -a $qcow_file --run-command "echo 'Modified from https://github.com/oneclickvirt/pve_kvm_images' >> /etc/motd"
    ...
    if [[ "$qcow_file" == *"debian"* || "$qcow_file" == *"ubuntu"* ]]; then
        virt-customize -a $qcow_file --run-command "apt-get install sudo -y"
    ...
    virt-customize -a $qcow_file --run-command "apk add --no-cache sudo"
    echo "启用SSH功能..."
    ...
    virt-customize -a $qcow_file --run-command "systemctl restart sshd"
    virt-customize -a $qcow_file --run-command "systemctl restart ssh"
    virt-customize -a $qcow_file --run-command "yum install sudo -y"
    echo "安装qemu-guest-agent..."
    ...
    virt-customize -a $qcow_file --run-command "systemctl restart ssh"
    echo "安装qemu-guest-agent..."
    virt-customize -a $qcow_file --run-command "dnf update -y && dnf install qemu-guest-agent -y"
    virt-customize -a $qcow_file --run-command "systemctl start qemu-guest-agent"
fi
virt-customize -a $qcow_file --run-command "echo root:oneclickvirt | chpasswd root"
virt-customize -a $qcow_file --run-command "echo root:oneclickvirt | sudo chpasswd root"
echo "创建备份..."
